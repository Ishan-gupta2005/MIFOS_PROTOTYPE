...
@scanner_bp.route('/api/mifos-gazelle/iac/scan', methods=['POST'])
def start_scan():
    data = request.json
    ...
    try:
        ...
        return jsonify({
            "scanId": scan_id,
            "results": findings,
            "message": "Mifos Gazelle IaC scan completed successfully with remediation guidance",
...
@scanner_bp.route('/api/mifos-gazelle/iac/export', methods=['GET'])
def export_scan_results():
    ...
    try:
        ...
        if format_type == 'csv':
            ...
            mimetype = 'text/csv'
            download_name = f"Mifos_Gazelle_IaC_Audit_{scan_id}.csv"
        else:
            file_path = export_to_pdf(results, filename=f"report_{scan_id}.pdf")
            mimetype = 'application/pdf'
            download_name = f"Mifos_Gazelle_IaC_Audit_{scan_id}.pdf"
