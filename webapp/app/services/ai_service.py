...
def get_ai_remediation(finding):
    ...
    # AI enrichment is optional for demo mode. If config is absent, return a
    # deterministic fallback without raising noisy runtime errors.
    if not SARVAM_API_KEY or not SARVAM_URL:
        return "AI remediation is disabled. Configure SARVAM_API_KEY and SARVAM_API_URL to enable it."

    description = finding.get("rule_description") or finding.get("description") or "Unknown finding"
    ...
    payload = {
        ...
        "messages": [
            ...
            {
                "role": "user", 
                "content": f"Fix this: {description}"
            }
        ],
