...
def export_to_pdf(results, filename="scan_report.pdf"):
    ...
    # Title
    elements.append(Paragraph("Mifos Gazelle IaC Security Audit Report", styles['Title']))
    elements.append(Paragraph(f"Prototype Context: Mifos Gazelle | Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", styles['Normal']))
