...
from mifos_iac.utils.reporting import export_to_csv, export_to_pdf
from mifos_iac import __version__

from mifos_iac.scanners.terraform import run_local_scan

console = Console()

# --- Ascii Art Banner ---
MIFOS_IAC_BANNER = r"""
 __  __ ___ _____ ___  ____     ____    _    _____ _____ _     _     _____ 
|  \/  |_ _|  ___/ _ \/ ___|   / ___|  / \  |__  /| ____| |   | |   | ____|
| |\/| || || |_ | | | \___ \  | |  _  / _ \   / / |  _| | |   | |   |  _|  
| |  | || ||  _|| |_| |___) | | |_| |/ ___ \ / /_ | |___| |___| |___| |___ 
|_|  |_|___|_|   \___/|____/   \____/_/   \_\____||_____|_____|_____|_____|

                     IaC Security Prototype for Mifos Gazelle Deployments
...
@click.version_option(__version__, '--version', '-V', prog_name="Mifos Gazelle IaC CLI")
def cli():
    """Mifos Gazelle IaC Security CLI Prototype.
...
@cli.command()
def version():
    """Display the CLI version and prototype context."""
    version_text = Text()
    version_text.append("Mifos Gazelle IaC CLI ", style="bold magenta")
    version_text.append(f"v{__version__}\n", style="bold cyan")
    version_text.append("Prototype for ", style="dim white")
    version_text.append("Mifos Gazelle", style="bold yellow")
    
    console.print(Panel(version_text, border_style="dim white", expand=False))

# --- HELPER FUNCTIONS ---
def print_banner():
    """Prints the Mifos IaC ASCII art banner."""
    styled_banner = Text(MIFOS_IAC_BANNER, style="bold magenta")
    console.print(Align(styled_banner, align="center"))
...
def iac(scan_path, scanners, format, csv_export, pdf_export):
    """Scans local IaC files (Terraform) for security issues in Mifos context."""
    ...
    console.print(Align(Text("Initializing Mifos Gazelle IaC Scan Engine...", style="bold cyan"), align="center"))
    print_banner()
    console.print("\n")
